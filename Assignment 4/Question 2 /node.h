...
class RoutingNode: public Node {
 public:
  void recvMsg(int curr, vector<RoutingNode*> nd, vector<bool> &visited_nodes, vector<int> &parent_node, vector<int> &distances, unordered_map<string, int> edge_cost);
  void sendMsg(int curr, vector<RoutingNode*> nd, unordered_map<string, int> edge_cost){
    ...
    for(int j=0; j<this->getInterfaceSize(); ++j){
      ...
      string e1=to_string(curr), e2=",", e3=to_string(adj);
      string e=e1+e2+e3;
      if(!visited_nodes[adj] and (distances[curr]+edge_cost[e])<distances[adj]){
        distances[adj]=distances[curr]+edge_cost[e];
        parent_node[adj]=curr;
      }
    }

    this->recvMsg(curr, nd, visited_nodes, parent_node, distances, edge_cost);
  }
};
